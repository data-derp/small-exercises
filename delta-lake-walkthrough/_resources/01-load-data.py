...
df = df.cache()
df.repartition(5).write.mode("overwrite").format("json").save(raw_data_location+"/user_json")
df.repartition(5).write.mode("overwrite").format("parquet").save(raw_data_location+"/user_parquet")